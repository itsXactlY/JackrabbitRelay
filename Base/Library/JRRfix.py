...
# -*- coding: utf-8 -*-
#
# Jackrabbit Relay, FIX framework
# 2021 Copyright © Robert APM Darin
# All rights reserved unconditionally.

# FIX is an event driven process, where as REST is a single request process. This FIX implementation is going to convert the
# standard FIX implementation into a quasi-REST version by having the event loop place one complete FIX sequence at a time
# with full login/logout.

# The impact of breaking down the TCP/IP connection is not different then REST Relay is acting as a bridge from REST
# (TradingView) to FIX (fxcm, 290+ others). This is a very ineffecient process, but required for TradingView and other simplar
# REST products to integrate properly with FIX based brokers for automated trading.
